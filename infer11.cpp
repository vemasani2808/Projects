...
int main(int argc, char* argv[]) {
  ...
  const TModel Model = (TModel)Env.GetIfArgPrefixInt("-m:", 1, "0:exponential, 1:power law, 2:rayleigh, 3:weibull");
  ...
  const int TakeAdditional = Env.GetIfArgPrefixInt("-s:", 5, "How much additional files to create?\n\
    0:no plots, 1:precision-recall plot, 2:accuracy plot, 3:mae plot, 4:mse plot, 5:all plots\n");

  bool PlotPrecisionRecall = true; 
  bool PlotAccuracy = true;
  bool PlotMAE = true;
  bool PlotMSE = true;
  
  bool PlotPerformance = true;
...
PlotPrecisionRecall = false; 
  ...
  for (int i=0; i<NIds.Len(); i++) {
   ...
   printf("varshini brings flowers to god");
